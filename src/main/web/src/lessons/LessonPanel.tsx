...
export function LessonPanel({ session, breadcrumb, verification, verifying, onVerify, onPrevious, onNext, onExit }: Props) {
  ...
  return <DockPane paneId="lesson" className={`lesson-panel${theory ? ' is-theory' : ''}`} label="Conteúdo da aula" headerClassName="bottom-tabs lesson-pane-header" header={<>{theory ? <LessonBreadcrumb breadcrumb={breadcrumb} /> : <span className="lesson-pane-kicker">Conteúdo da aula</span>}<span className="lesson-part-indicator">Parte {session.currentStep + 1} de {session.totalSteps}</span></>} bodyClassName="lesson-pane-content lesson-panel-content learning-body" footerClassName="lesson-panel-actions" footer={<><button type="button" onClick={onExit}>Voltar ao roteiro</button><div><button type="button" onClick={onPrevious} disabled={!session.canPrevious}>Anterior</button><button type="button" className="primary-action" onClick={onNext} disabled={!session.canNext}>Próximo</button></div></>}>
      <article className="lesson-reading-article">{theory && <header className="lesson-chapter-header"><h1 className="lesson-chapter-title">{session.title}</h1>{session.message && <p className="lesson-chapter-lead">{session.message}</p>}</header>}<LessonBlocks blocks={session.contentBlocks} theory={theory} /></article>
