---
id: java/jdk/string/last-index-of
title: String.lastIndexOf()
concept: string-last-index-of
kind: method
sourceMember: lastIndexOf
sourceSignature: (String)
level: beginner
duration: 2
category: API JAVA
parent: java/jdk/string
related:
  - java/jdk/string
---
## O que ele faz?

Usa lastIndexOf() em String para realizar a operação indicada pela API Java.

```java
String texto = "EyeCode";
var resultado = texto.lastIndexOf("e");
```

## Comportamento

O resultado e os efeitos dependem dos argumentos e do contrato de String. Verifique o retorno antes de continuar quando a operação puder indicar ausência, falha ou mudança de estado.
