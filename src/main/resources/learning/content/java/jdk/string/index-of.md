---
id: java/jdk/string/index-of
title: String.indexOf()
concept: string-index-of
kind: method
sourceMember: indexOf
sourceSignature: (String)
level: beginner
duration: 2
category: API JAVA
parent: java/jdk/string
related:
  - java/jdk/string
---
## O que ele faz?

Usa indexOf() em String para realizar a operação indicada pela API Java.

```java
String texto = "EyeCode";
var resultado = texto.indexOf("Code");
```

## Comportamento

O resultado e os efeitos dependem dos argumentos e do contrato de String. Verifique o retorno antes de continuar quando a operação puder indicar ausência, falha ou mudança de estado.
