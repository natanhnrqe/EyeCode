...
class MonacoOverlayResourceTest {
    ...
    void editorPageContainsPersistentContextLayerAndGenerationGuard() throws IOException {
        try (InputStream stream = getClass().getResourceAsStream("/monaco/editor/index.html")) {
            ...
            assertTrue(page.contains("registerCompletionItemProvider('java'"));
            assertTrue(page.contains("eyeCodeCompletionProvider"));
            ...
            assertTrue(page.contains("EYECODE_ORIGINAL_REQUEST"));
            assertTrue(page.contains("EYECODE_ORIGINAL_RESULT"));
            assertTrue(page.contains("EYECODE_BRIDGE_SEND"));
            assertTrue(page.contains("content: model.getValue()"));
            assertTrue(page.contains("CompletionItemInsertTextRule.InsertAsSnippet"));
            assertTrue(page.contains("wordBasedSuggestions: false"));
            assertTrue(page.contains("quickSuggestions: { other: true, comments: false, strings: false }"));
            assertTrue(page.contains("suggestOnTriggerCharacters: true"));
            assertTrue(page.contains("triggerCharacters: ['.']"));
